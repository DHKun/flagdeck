...
- **表单驱动运行**：curl / arjun / dddd / fscan / ffuf / gobuster 等按各自 CLI 定制字段；场景卡片、目标记忆、工具搜索。
- **结果表**：ffuf 等 JSON/文本旁路输出可解析为表格（日志 / 结果双 Tab）。
- **独立应用入口**：ShiroExploit、AntSword、Behinder、Godzilla、UploadRanger 等一键启动（需本机安装对应客户端）。
- **统一任务与日志**：stdout / stderr 写入本地工作区，可筛选、复制、删除与清空。
