...
export function toHexPreview(value: string, maximumBytes = 256): string {
  if (!Number.isSafeInteger(maximumBytes) || maximumBytes < 0) {
    throw new RangeError("invalid byte limit");
  }
  const bytes = new TextEncoder().encode(value);
  const clipped = bytes.subarray(0, maximumBytes);
  const rendered = Array.from(clipped, (byte) =>
    byte.toString(16).padStart(2, "0"),
  ).join(" ");
  return bytes.length > maximumBytes ? `${rendered} …` : rendered;
}
