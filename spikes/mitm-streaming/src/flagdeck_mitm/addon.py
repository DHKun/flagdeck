...
class StreamingCaptureAddon:
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        self._finalize(flow, "request", "request")

    def response(self, flow: http.HTTPFlow) -> None:
        self._finalize(flow, "response", "response")
    ...
    def client_disconnected(self, client: connection.Client) -> None:
        with self._captures_lock:
            ...
            pending = [(key, self._captures.pop(key)) for key in keys if key in self._captures]
            for key in keys:
                self._capture_clients.pop(key, None)
        for _key, capture in pending:
            try:
                result = capture.abort("client_disconnected")
            except StrictCaptureError:
                result = capture.result()
            self._emit_result(result, hook="client_disconnected")

    def done(self) -> None:
        with self._captures_lock:
            pending = list(self._captures.items())
        ...
        for _key, capture in pending:
            try:
                ...
                result = capture.result()
            self._emit_result(result, hook="done")
    ...
    def _finalize(
        ...
        *,
        ...
        try:
            ...
            result = capture.result()
        self._emit_result(result, hook=hook)

    def _emit_result(self, result: CaptureResult, *, hook: str) -> None:
